  ...
  controller.on("bot_space_join", function(bot, event) {
    ...
    bot.say(
      ...
      function(err, rawMessage) {
        if (err) {
          console.log(
            "Error while posting back welcome message, err: " + err.message
          );
          return;
        }
        ...
        if (rawMessage.roomType == "group") {
          help =
            ...
            bot.appendMention(rawMessage, "help");
        }

        help +=
          "<br/>Mais cherchez pas à faire des phrases pourries... On en a gros, c'est tout !";
        ...
        bot.say(
          ...
          function(err, messageAck) {
            if (err) {
              console.log(
                "Error while postig back help message, err: " + err.message
              );
              return;
            }
          }
